...
def load_dataset_desc(dataset_name: str) -> str:
    ...
    # read json file: support both datasets/{name}/desc.json and datasets/data_source/{name}/desc.json
    candidates = [
        f'datasets/{dataset_name}/desc.json',
        f'datasets/data_source/{dataset_name}/desc.json',
    ]
    last_err = None
    for p in candidates:
        try:
            ...
            last_err = e
            continue
    raise last_err if last_err is not None else FileNotFoundError(f"desc.json for {dataset_name} not found in {candidates}")
...
def load_dataset_data(dataset_name: str) -> np.ndarray:
    ...
    shape = desc['shape']
    candidates = [
        f'datasets/{dataset_name}/data.dat',
        f'datasets/data_source/{dataset_name}/data.dat',
    ]
    last_err = None
    for dat_file_path in candidates:
        try:
            ...
            last_err = e
            continue
    raise last_err if last_err is not None else FileNotFoundError(f"data.dat for {dataset_name} not found in {candidates}")
