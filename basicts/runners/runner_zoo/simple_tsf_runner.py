...
class SimpleTimeSeriesForecastingRunner(BaseTimeSeriesForecastingRunner):
    ...
    def postprocessing(self, input_data: Dict) -> Dict:
        ...
        # input_data['prediction_unnorm'] = self.scaler.inverse_transform(input_data['prediction'])
        # input_data['target_unnorm'] = self.scaler.inverse_transform(input_data['target'])
        # input_data['inputs_unnorm'] = self.scaler.inverse_transform(input_data['inputs'])
        ...
        if self.scaler is not None and self.scaler.rescale:
            ...
            input_data['target'] = self.scaler.inverse_transform(input_data['target'])
            input_data['inputs'] = self.scaler.inverse_transform(input_data['inputs'])
            input_data['prediction_unnorm'] = input_data['prediction']
            input_data['target_unnorm'] = input_data['target']
            input_data['inputs_unnorm'] = input_data['inputs']
        else:
            input_data['prediction_unnorm'] = self.scaler.inverse_transform(input_data['prediction'])
            input_data['target_unnorm'] = self.scaler.inverse_transform(input_data['target'])
            input_data['inputs_unnorm'] = self.scaler.inverse_transform(input_data['inputs'])
