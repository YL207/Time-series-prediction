...
class BaseEpochRunner(metaclass=ABCMeta):
    ...
    def __init__(self, cfg: Dict) -> None:
        ...
        # param (need to set ckpt_save_dir early for logger)
        ...
        # 获取数据集名称，确保每个数据集有独立的日志文件
        dataset_name = get_dataset_name(cfg)
        log_file_name = f'training_log_{dataset_name}_{time.strftime("%Y%m%d%H%M%S", time.localtime())}.log'
        log_file_path = os.path.join(self.ckpt_save_dir, log_file_name)
        
        # 默认logger - 使用包含数据集名称的日志文件
        self.logger = get_logger('easytorch', log_file_path)
    ...
    def on_training_end(self, cfg: Dict, train_epoch: Optional[int] = None):
        ...
        if hasattr(cfg, 'TEST'):
            ...
            if hasattr(self, 'evaluate_val'):
                self.logger.info('Evaluating the best model on the validation set.')
                self.evaluate_val(save_metrics=True, save_results=True)
            self.test_pipeline(cfg=cfg, train_epoch=train_epoch, save_metrics=True, save_results=True)
            
            # evaluate the best model on the training set
            if hasattr(self, 'evaluate_train'):
                self.logger.info('Evaluating the best model on the training set.')
                self.evaluate_train(save_metrics=True, save_results=True)
